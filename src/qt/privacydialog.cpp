                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zXORX ought to be enough for anybody." - Bill Gates, 2017
    ui->zXORXpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzXORXSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzXORX->setEnabled(false);
        ui->pushButtonMintzXORX->setToolTip(tr("zXORX is currently disabled due to maintenance."));

        ui->pushButtonSpendzXORX->setEnabled(false);
        ui->pushButtonSpendzXORX->setToolTip(tr("zXORX is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zXORXpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzXORX_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXORX is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zXORX...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zXORX in ") +
...
void PrivacyDialog::on_pushButtonSpendzXORX_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zXORX is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zXORX
        sendzXORX();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zXORX
    sendzXORX();
}

void PrivacyDialog::on_pushButtonZXORXControl_clicked()
{
    ZXORXControlDialog* zXORXControl = new ZXORXControlDialog(this);
    zXORXControl->setModel(walletModel);
    zXORXControl->exec();
}

void PrivacyDialog::setZXORXControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzXORXSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzXORX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid XORX Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zXORXpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zXORX is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " XORX </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zXORX</b>";
    ...
    // use mints from zXORX selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZXORXControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZXORXControlDialog::GetSelectedMints();
    }
    ...
    // Clear zxorx selector in case it was used
    ZXORXControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zXORX Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " XORX, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zXORX Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zXORX </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zXORX "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zXORX "));
    ui->labelzXORXAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzXORXSyncStatus->setVisible(fShow);
}
