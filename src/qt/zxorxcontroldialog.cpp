...
#include "ui_zxorxcontroldialog.h"
...
std::list<std::string> ZXORXControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZXORXControlDialog::listMints;

ZXORXControlDialog::ZXORXControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZXORXControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZXORXControlDialog::~ZXORXControlDialog()
{
    delete ui;
}

void ZXORXControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZXORXControlDialog::updateList()
{
...
void ZXORXControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZXORXControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZXORX_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZXORXControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZXORXControlDialog::GetSelectedMints()
{
...
void ZXORXControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
