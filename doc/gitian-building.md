...
*Setup instructions for a gitian build of XORX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the XORX
...
Clone the git repositories for xorx and gitian and then checkout the xorx version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the xorx repository
...
To build XORX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the xorx repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xorx/.git/
...
./bin/gbuild --commit xorx=${COMMIT} --url xorx=${URL} ../XORX/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xorx=${COMMIT} --url xorx=${URL} ../XORX/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xorx=${COMMIT} --url xorx=${URL} ../XORX/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xorx-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xorx-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/xorx-build.assert
```
