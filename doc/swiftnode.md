...
This mechanism has been integrated into Bitcore-Node-XORX which allows for notification to be broadcast through Insight API in one of two ways:
* WebSocket: 
* API:  
...
By default, the XORX Core daemon will launch using the following constant:

```
static const int DEFAULT_SWIFTNODE_DEPTH = 5;
```

This value can be overridden by passing the following argument to the XORX Core daemon:
