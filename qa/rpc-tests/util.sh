...
# JSON produced by xorxd, do NOT use it to try to
...
function CreateTxn1 {
  ...
  RAWTXORX=$( $CLI $1 createrawtransaction "[{\"txid\":\"$TXID\",\"vout\":$VOUT}]" "{\"$3\":$AMOUNT}")
  ExtractKey hex "$( $CLI $1 signrawtransaction $RAWTXORX )"
}
